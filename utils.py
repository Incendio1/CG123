...
def OPP(nodenum, K, adjlist, edge_index):
    ...
    hop_edge_index = [np.zeros((2, nodenum**2), dtype=int) for i in range(K)]
    hop_edge_att = [np.zeros(nodenum**2) for i in range(K)]
    hop_edge_pointer = np.zeros(K, dtype=int)

    for i in range(nodenum):
        hop_edge_index, hop_edge_att, hop_edge_pointer = Width_adjlist(adjlist, nodenum, hop_edge_index, hop_edge_att, hop_edge_pointer, att, source=i, depth_limit=K)
...
def Width_adjlist(adjlist, nodenum, hop_edge_index, hop_edge_att, hop_edge_pointer, deg_att, source, depth_limit):
    visited = {}
